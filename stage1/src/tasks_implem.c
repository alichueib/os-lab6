...
pthread_cond_t all_ready_done = PTHREAD_COND_INITIALIZER; // producer thread waits for all the ready tasks to be done, before enqueueing any new tasks in buffer. A thread checks if NbCreatedTasks = NbTerminatedTasks, if so it signals the main thread to wake up and resume production.
