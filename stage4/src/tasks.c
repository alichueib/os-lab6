...
{
    task_waitall(); //call it for the last time
...
task_t* create_task(task_routine_t f)
{
    ...
    t->task_dependency_done  = 0; //Now we initiallize it in case we have dependency. Try to remove it? 
    t->parent_task = NULL;
#endif
...
{
    PRINT_DEBUG(10, "Submitting task %u\n", t->task_id);   
    t->status = READY;
